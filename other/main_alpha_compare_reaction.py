...
import argparse
import os.path
from utils_reaction_mu import prepare_data
...
parser = argparse.ArgumentParser(description='HyperLink Prediction with Walk-Pooling')
#Dataset 
...
parser.add_argument('--alpha',type=str, default='b',
                    help='the configuration of alpha')
...
def RunExp(args):
    ...
    if (args.alpha == 'a'):
        tindex = [0, 1, 2, 3, 4, 32, 33, 34]  # alpha \in [0,1]
    elif (args.alpha == 'b'):
        tindex = [0, 1, 2, 3, 4, 17, 18, 19, 32, 33, 34]  # {0, 0.5, 1}
    elif (args.alpha == 'c'):
        tindex = [0, 1, 2, 3, 4, 11, 12, 13, 23, 24, 25, 32, 33, 34]  # {0, 0.3, 0.7, 1}
    elif (args.alpha == 'd'):
        tindex = [0, 1, 2, 3, 4, 8, 9, 10, 14, 15, 16, 20, 21, 22, 26, 27, 28, 32, 33, 34]  # {0, 0.2, 0.4, 0.6, 0.8, 1}
    else:
        tindex = range(w_len)
...
if __name__ == '__main__':
    ...
    file_out = args.data + '_' + args.alpha + '_' + str(mean_f1) + '_' + str(std_f1) + '_' + str(mean_auc) + '_' + str(std_auc) + '.txt'
